...
- LLM(Large Language Model)은 방대한 양의 텍스트 데이터를 학습하여 자연어(NLP)를 이해하고 생성할 수 있는 AI 모델입니다. 주로 딥러닝 기반의 신경망 아키텍처(Transformer)를 활용하며, 이를 통해 문장 생성, 번역, 텍스트 요약, 질문 응답과 같은 작업을 수행할 수 있습니다. [4]
- ChatGPT를 비롯하여 많은 LLM모델들이 등장하였다. 
Google의 PaLM(Pathways Language Mode), Meta의 LLaMA (Large Language Model Meta AI), NVIDIA의 Megatron-Turing NLG)등이 있다.
...
- 32-bit LLM 작동 원리
    ...
    - LLM 모델 추론 성공
        - 이렇듯, 가벼운 RNN 의 예시를 바탕으로 병렬 연산의 발생 가능성을 보여주었다.
...
- FP(Floating-point),FP32는 Single Precision이라고도 한다. 일반적으로 모델 학습에 사용되는 부동 소수점 형식이다. 최근 연구들에서 모델 사이즈가 점점 커지면서 계산량과 필요한 메모리의 크기가 기하급수적으로 커지는 것을 확인할 수 있다. 그런 점에서 이를 학습시키기 위해서 많은 리소스가 필요해지며 학습도 오래걸리는 현상이 발생했다. 이러한 문제점을 해결하기 위해서 Half-precision(FP16)을 사용하여 계산량과 메모리 사용량을 줄이는 방안이 제시되었다.

## FP16 [5]

- FP16는 Half-precision이라고 하며, 16비트 부동 소수점 형식을 나타낸다. . 그러나 16비트의 정밀도가 낮아서 모델의 정확도는 떨어질 수 있다.

![FP16의 형식](image%2010.png)
...
- 기존의 FP32으로 작동되던 코드들을 FP16으로 변환하게 되었을 때 training loss가 중간에 크게 증가하는 모습을 볼 수 있습니다. 그 이유로 역전파 과정에서 기울기 (Gradient)들을 계산하고 축적하여 가중치 값을 업데이트하는데, FP16을 사용하게 되면 표현할 수 있는 수의 범위가 적어지고 오차가 발생하고, 오차가 누적되면서 학습이 제대로 되지 않는 모습을 볼 수 있었습니다. 이 점을 해결하기 위해 학습과정에서 FP32와 FP16을 모두 사용하는 Mix-precision이 등장하였습니다.

![image.png](image%2012.png)

- Master-Weights를 FP32 에서 Fp16으로 변환한 뒤 정상파(Forward propagation)과 역전파(Backward propagation)은 모두 FP16으로 연산이 수행된다. 이후 가중치 값을 업데이트하는 과정에서 다시 FP32로 변환되어 가중치 값이 업데이트 됩니다.

![SSD Object Detector를 학습시킬 때 Activation gradients들의 크기의 빈도](image%2013.png)

SSD Object Detector를 학습시킬 때 Activation gradients들의 크기의 빈도
...
- 딥러닝에서는 반드시 정밀도가 높은 데이터 타입이 아니더라도 학습성능에 크게 영향을 주지 않는 다는 사실이 밝혀지면서 이 논리를 극대화 시키기 위해 FP8이 제안되었다. 이를 통해 학습능력에 크게 영향을 미치지 않으면서도 연산 효율성을 극대화합니다.

![image.png](image%2014.png)

- FP8에는 두 가지 인코딩 방식이 존재한다. 가중치 및 활성화 텐서에 대해서는 E4M3를, 기울기 텐서에 대해서는 E5M2를 사용하는 것을 권장한다.

![image.png](image%2015.png)

- (DeiT는 Transformer 기반 아키텍처이며, 나머지 모델은 CNN입니다. MobileNet v2를 제외하고 FP8을 사용한 훈련에서 크게 기존의 정확도와 차이가 크지 않은 모습을 볼 수 있다.)

![image.png](image%2016.png)

![image.png](image%2017.png)

- 주요 동기는 딥러닝 훈련 및 추론의 가속화로, 메모리 대역폭 압박을 줄이면서 더 작고 전력 효율적인 수학 파이프라인을 가능하게 하며 이미지 및 언어 작업을 위한 다양한 신경망 모델을 FP8에서 훈련하여 동일한 모델, 최적화 도구 및 훈련 하이퍼파라미터를 사용하여 16비트 훈련 세션에서 달성된 모델 정확도와 일치시킬 수 있음을 보여줍니다.
...
- $W$는 $γ+ϵ$로 나누어 RoundClip함수에 대입하여 quatization시킨다.
...
- parameter들의 크기를 키우면서 모델의 latency와 memory 사용량을 측정한 결과, 크기가 커질 수록 격차가 점점 심해지는 것을 보여주며, 본 논문에서는 앞으로 모델 거대화있어서 더욱 효과적일 것이라고 실험적으로 보여주며 주장하고 있다. [10]
...
- 텍스트 생성 언어모델의 성능 평가지표 중 하나
- 언어 모델의 분기계수(Branching Factor)라고도 함
- 이전 단어로 다음 단어를 예측할때 “**몇개의 단어 후보를 고려”하는지** 의미
- Perplexity값이 낮을수록 성능이 우수 [11]

### Perplexity 비교 : FP16 LLaMA VS 1Bit LLM

- 비교대상 : FP16 LLaMA → 기존 LLM에서 파라미터를 표현하는 방식
- FP16 : 16비트로 구성된 부동소수점(floating-point)으로 **파라미터 표현**

![image.png](image%2021.png)

- **Memory (GB), Latency(ms), Perplexity(PPL) 비교 [12]**
- **메모리(GB)**: BitNet b1.58는 FP16 LLaMA 대비 메모리 사용량이 **2.6배~3.55배 감소**
- **지연 시간(Latency)**: BitNet b1.58는 FP16 LLaMA 대비 **1.23배~2.71배 빠름**
- **Perplexity(PPL)**: 성능 손실은 최소화되었으며, 특히 **3B 모델(9.91)** 및 **3.9B 모델(9.62)**에서 LLaMA보다 낮은 PPL 값을 보임

![image.png](image%2022.png)

- **Zero-shot(모델이 학습 과정에서 배우지 않은 작업을 수행하는 것) 비교 [b]**
- **3B 모델 크기** 이상에서는
    - BitNet b1.58이 **FP16 LLaMA와 유사하거나 더 나은 성능**을 보임.
    - 특히 **ARCc(28.7)**, **HS(44.2)**, **PQ(73.2)** 등의 과제에서 두드러진 성능 향상을 기록.

### 결론

- Perplexity와 Zero-shot 정확도 모두에서 BitNet b1.58은 **LLaMA와 경쟁력 있는 성능**
- 특히 3B 이상 모델에서 FP16 LLaMA를 능가하거나 동등한 퍼포먼스
...
- 허깅 페이스 ,  MS의 오픈소스 활용
- 1bit LLM과 32bit LLM의 토큰 별 추론 시간 비교
- 10,20,50,100,200,500,1000 토큰 별 10회 추론 진행 후 **평균 추론 시간 도출 및 대조**
...
- 1bit LLM의 평균 추론 시간이 32bit LLM 대비 2 ~ 5배 더 빠름
- 토큰 값이 커질수록 차이가 극명하게 나타남

---

## 결론 (1-bit LLM의의 및 마무리)

- 기존 병렬 연산이 주가 되는 AI 모델에 대하여 1-bit LLM 기술을 적용하게 되면서, 1) 모델의 크기(파라미터)가 줄어들었으며, 2) 추론 시간의 효율성 또한 높아졌다.
- 하지만, 지금은 순서대로 값을 집어넣는 것이 아닌, 사진과 같이 2차원적인 성격이 섞여 있는 상황에서는 1-bit 로 줄어드는 과정 중 정보 손실이 일어날 수 있어 이러한 부분은 개선해야 할 방향이라고 보여진다. 이러한 필요성이 대두되는 상황 중 4-bit 활성화 함수에 대한 연구도 최근 활발하게 이뤄지는 것으로 확인되었다. [3]
...
- 위 사례와 같이 연구 가치가 있는 기술이라고 생각하며, 앞으로의 연구에 관심을 가졌으면 하는 말을 마지막으로 글을 마치고자 한다.
